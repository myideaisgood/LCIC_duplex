...
unsigned Arithmetic_Codec::get_code_bytes(void)
{
	unsigned code_bytes = unsigned(ac_pointer - code_buffer);
	if (code_bytes > buffer_size) AC_Error("code buffer overflow");
...
void Adaptive_Data_Model::set_distribution(float ratio)
{
	float *probability = new float[data_symbols];

	float ss = probability[0] = 1;
	for (unsigned k = 1; k < data_symbols; k++) {
		probability[k] = ratio * probability[k - 1];
		ss += probability[k];
	}
	...
	for (unsigned k = 0; k < data_symbols; k++) {
		...
		if (probability[k] < 0.0001) {
			a += 0.00011 - probability[k];
			probability[k] = 0.00011;
		}
	}
	probability[0] -= a;
